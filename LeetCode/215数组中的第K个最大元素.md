...
class Solution {
    ...
    // 三向切分
    int[] partiton(int[] nums, int i, int j) {
        int pivot = nums[i];
        // [i, l)严格小于pivot的元素, (r, j]严格大于pivot的元素, [l, k)等于pivot的元素
        int l = i, r = j, k = i;
        while (k <= r) {
            if (nums[k] < pivot) {
                swap(nums, l++, k++);
    ...
    int quickSelect(int[] nums, int kSmall) {
        int i = 0, j = nums.length - 1;
        while (i <= j) {
            int[] bounds  = partiton(nums, i, j);
